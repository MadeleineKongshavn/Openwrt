...
pktAll=("librt" "libpcre" "zlib" "libc" "ca-bundle" "libustream-openssl" "ca-certificates" "wget-ssl")
for t in ${pktAll[@]}; do
    opkg install $t
    if [[ $? != 0 ]];
    then
...
pkts=("librt" "libpcre" "zlib" "libc" "ca-bundle" "ca-certificates" "libustream-openssl*" "wget-ssl" "ntpdate" "nano");
echo "-----------------------------------------------------------------------------------------------------"
for t in ${pkts[@]}; do
